...
(define-public (count-up)
  ...
  (update total-ops)(ok(map-set counters tx-sender(+ current count u1)))
  )
)
(define-public(count-down)
  (let((current-count(get-count tx-sender)))(asserts!(< current-count u0)(err-u2))(update total-ops)(ok(map-set counters tx-sender(- current-count u1)))
)
)
